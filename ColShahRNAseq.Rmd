...
Get the data.
```{r}
counts <- read.csv("ColShahCounts.csv.gz",row.names = 1)
head(counts[,1:10])
...
Remove samples with <= 1M counts, replot

```{r}
counts <- counts[,colSums(counts) > 1e06]
pl.orig2 <- counts %>% colSums() %>% tibble(sample=names(.),count=.) %>%
  ggplot(aes(x=sample,y=count)) + 
  geom_col() +
  theme(axis.text.x  = element_text(angle=90, vjust=0.5,size = 7)) 
pl.orig2
...
samples <- samples %>% 
  separate(sample,
           ...
           remove=FALSE) %>%
  select(-extra) %>%
  mutate(gt=ifelse(gt=="22","Col","Shah"), group=str_c(gt,trt,time,sep="_"))
samples
```
...
for(t in unique(samples$time)) {
  ...
  counts.subset <- counts[,str_detect(colnames(counts),str_c("[HL]",t,"A$"))]
...
print(plotMDS(dge.subset,method = "bcv",labels=dge.subset$samples$group))
}
